...
if !(_exit) exitWith {[(format [localize "STR_Shop_Veh_NoLicense"]),"Hinweis","Yellow"] call MSG_fnc_handle;};
...
//Cop / Civ Pre Check
if (_shop in ["bruce","dive","reb","kart"] && {!(playerSide isEqualTo civilian)}) exitWith {[(format [localize "STR_Shop_NotaCiv"]),"Hinweis","Yellow"] call MSG_fnc_handle; closeDialog 0;};
if (_shop == "reb" && {!license_civ_rebel}) exitWith {[(format [localize "STR_Shop_NotaReb"]),"Hinweis","Yellow"] call MSG_fnc_handle;closeDialog 0;};
if (_shop == "cop" && {!(playerSide isEqualTo west)}) exitWith {[(format [localize "STR_Shop_NotaCop"]),"Hinweis","Yellow"] call MSG_fnc_handle; closeDialog 0;};
if (_shop == "dive" && {!license_civ_dive}) exitWith {[(format [localize "STR_Shop_NotaDive"]),"Hinweis","Yellow"] call MSG_fnc_handle; closeDialog 0;};
...
if (isClass(missionConfigFile >> "Licenses" >> life_clothing_store)) then {
    ...
    if !(LICENSE_VALUE(life_clothing_store,_flag)) exitWith {
        [(format [localize "STR_Shop_YouNeed",localize _displayName]),"Hinweis","Yellow"] call MSG_fnc_handle;
        closeDialog 0;
    };
};
