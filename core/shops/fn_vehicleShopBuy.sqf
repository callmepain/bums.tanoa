...
if (_mode) then 
{
    if (_insure) then 
	{
		[(format["Du hast ein<br/><t color='#FFFF00'>%1<t><br/><t color='#FFFFFF'> für <t color='#FFFF00'>$%2<t> <t color='#FFFFFF'>inkl. Versicherung gekauft.<br/><br/>Bitte warten.<br/>Der zuständige Mitarbeiter such noch den Schlüssel.<t>",getText(configFile >> "CfgVehicles" >> _className >> "displayName"),[_purchasePrice] call life_fnc_numberText]),"Fahrzeug kauf","green"] call MSG_fnc_handle;
	}
	else
	{
		[(format["Du hast ein<br/><t color='#FFFF00'>%1<t><br/><t color='#FFFFFF'>für <t color='#FFFF00'>$%2<t> <t color='#FFFFFF'>gekauft.<br/>Bitte warten.<br/><br/>Der zuständige Mitarbeiter such noch den Schlüssel.<t>",getText(configFile >> "CfgVehicles" >> _className >> "displayName"),[_purchasePrice] call life_fnc_numberText]),"Fahrzeug kauf","green"] call MSG_fnc_handle;
	};
} 
else
{
	[(format["Du hast ein<br/><t color='#FFFF00'>%1<t><br/><t color='#FFFFFF'>für<t> <t color='#FFFF00'>$%2<t> <t color='#FFFFFF'>gemietet.<br/>Bitte warten.<br/><br/>Der zuständige Mitarbeiter such noch den Schlüssel.<t>",getText(configFile >> "CfgVehicles" >> _className >> "displayName"),[_purchasePrice] call life_fnc_numberText]),"Fahrzeug kauf","green"] call MSG_fnc_handle;
};
