...
if !(playerSide isEqualTo civilian) exitWith {[(format [localize "STR_NOTF_notAllowed"]),"Hinweis","Yellow"] call MSG_fnc_handle;};
...
//Error check
if (count _nearVehicles isEqualTo 0) exitWith {[(format [localize "STR_Shop_NoVehNear"]),"Hinweis","Yellow"] call MSG_fnc_handle;};
if (!(createDialog "Chop_Shop")) exitWith {[(format [localize "STR_Shop_ChopShopError"]),"Hinweis","Yellow"] call MSG_fnc_handle;};
