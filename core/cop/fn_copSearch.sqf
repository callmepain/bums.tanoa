...
if (!alive _civ || player distance _civ > 5) exitWith {[(format [localize "STR_Cop_CouldntSearch",_civ getVariable ["realname",name _civ]]),"Hinweis","Yellow"] call MSG_fnc_handle;	};
//hint format ["%1",_this];
[(format ["<t color='#FF0000'><t size='2'>%1</t></t><br/><t color='#FFD700'><t size='1.5'><br/>" +(localize "STR_Cop_IllegalItems")+ "</t></t><br/>%2<br/><br/><br/><br/><t color='#FF0000'>%3</t>"
,(_civ getVariable ["realname",name _civ]),_inv,if (_robber) then {"Robbed the bank"} else {""}]),"Hinweis","Yellow"] call MSG_fnc_handle;	
