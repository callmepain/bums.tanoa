...
if (CASH < life_ticket_val) exitWith {
    if (BANK < life_ticket_val) exitWith {
        [(format [localize "STR_Cop_Ticket_NotEnough"]),"Hinweis","Yellow"] call MSG_fnc_handle;	
        [1,"STR_Cop_Ticket_NotEnoughNOTF",true,[profileName]] remoteExecCall ["life_fnc_broadcast",life_ticket_cop];
        closeDialog 0;
    };

    [(format [localize "STR_Cop_Ticket_Paid",[life_ticket_val] call life_fnc_numberText]),"Hinweis","Green"] call MSG_fnc_handle;	
