...
if (count _vehicleInfo isEqualTo 0) exitWith {[(format [localize "STR_Cop_VehEmpty"]),"Hinweis","green"] call MSG_fnc_handle;	};
...
if (_value > 0) then {
    ...
    [(format [localize "STR_Cop_NoIllegalVeh"]),"Hinweis","Green"] call MSG_fnc_handle;	
};
