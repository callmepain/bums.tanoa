...
if (_isVehicle && _curTarget in life_vehicles) exitWith {[(format [localize "STR_ISTR_Lock_AlreadyHave"]),"Hinweis","Yellow"] call MSG_fnc_handle;};

//More error checks
if (!_isVehicle && !isPlayer _curTarget) exitWith {};
if (!_isVehicle && !(_curTarget getVariable ["restrained",false])) exitWith {};
if (_curTarget getVariable "NPC") exitWith {[(format [localize "STR_NPC_Protected"]),"Hinweis","Yellow"] call MSG_fnc_handle;};
...
if (!_isVehicle) then {
    ...
    _dice = random(100);
    if (_dice < 30) then {
        [(format [localize "STR_ISTR_Lock_Success"]),"Hinweis","Green"] call MSG_fnc_handle;
        ...
        [(format [localize "STR_ISTR_Lock_Failed"]),"Hinweis","Yellow"] call MSG_fnc_handle;
    };
};
