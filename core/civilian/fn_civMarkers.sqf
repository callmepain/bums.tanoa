...
for "_i" from 0 to count(_gatherCfg)-1 do {
	...
	_gather pushback [_gatherZones,_gatherName,_gatherLevel];
};
    ...
    {
		{
			if ((_x select 2) <= (_data select 0)) then {
				...
				for "_f" from 0 to ((count _test) - 1) do {
					_marker2 = createMarkerLocal [format ["%1_mine_%2",localize(_x select 1),_f], getMarkerPos (_x select 0 select _f)];
					...
					_marker3 = createMarkerLocal [format ["%1_name_%2",localize(_x select 1),_f], getMarkerPos (_x select 0 select _f)];
		...
		{
			if ((_x select 2) <= (_data2 select 0)) then {
			_test2 = (_x select 0);
				for "_z" from 0 to ((count _test2) - 1) do {
					_marker6 = createMarkerLocal [format ["%1_gather_%2",localize(_x select 1),_z], getMarkerPos (_x select 0 select _z)];
					...
					_marker7 = createMarkerLocal [format ["%1_gathername_%2",localize(_x select 1),_z], getMarkerPos (_x select 0 select _z)];
					_marker7 setMarkerTypeLocal "mil_triangle";
					_marker7 setMarkerColorLocal "ColorGreen";
					_marker7 setMarkerTextLocal format [localize (_x select 1)];
