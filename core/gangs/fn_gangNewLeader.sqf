...
if ((lbCurSel 2621) isEqualTo -1) exitWith {[(format [localize "STR_GNOTF_TransferSelect"]),"Hinweis","Yellow"] call MSG_fnc_handle;};
_unit = call compile format ["%1",CONTROL_DATA(2621)];

if (isNull _unit) exitWith {}; //Bad unit?
if (_unit == player) exitWith {[(format [localize "STR_GNOTF_TransferSelf"]),"Hinweis","Yellow"] call MSG_fnc_handle;};
...
if (_action) then {
    ...
    if (_unitID isEqualTo "") exitWith {[(format [localize "STR_GNOTF_badUID"]),"Hinweis","Yellow"] call MSG_fnc_handle;}; //Unlikely?
    ...
    [(format [localize "STR_GNOTF_TransferCancel"]),"Hinweis","Yellow"] call MSG_fnc_handle;
};
