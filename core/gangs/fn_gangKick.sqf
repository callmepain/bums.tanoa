...
if ((lbCurSel 2621) isEqualTo -1) exitWith {[(format [localize "STR_GNOTF_SelectKick"]),"Hinweis","Yellow"] call MSG_fnc_handle;};
_unit = call compile format ["%1",CONTROL_DATA(2621)];

if (isNull _unit) exitWith {}; //Bad unit?
if (_unit == player) exitWith {[(format [localize "STR_GNOTF_KickSelf"]),"Hinweis","Yellow"] call MSG_fnc_handle;};
