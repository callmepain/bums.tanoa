...
if (_action) then {
    if (BANK < _upgradePrice) exitWith {
        ...
        ]),"Hinweis","Yellow"] call MSG_fnc_handle;
    };
    BANK = BANK - _upgradePrice;
    [1] call SOCK_fnc_updatePartial;
    group player setVariable ["gang_maxMembers",_slotUpgrade,true];
    [(parseText format [localize "STR_GNOTF_UpgradeSuccess",_maxMembers,_slotUpgrade,[_upgradePrice] call life_fnc_numberText]),"Hinweis","Yellow"] call MSG_fnc_handle;
    ...
    [(format [localize "STR_GNOTF_UpgradeCancel"]),"Hinweis","Yellow"] call MSG_fnc_handle;
};
