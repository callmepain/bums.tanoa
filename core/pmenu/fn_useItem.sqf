...
switch (true) do {
        ...
        if ([false,_item,1] call life_fnc_handleInv) then {
            ...
            if (_item isEqualTo "redgull" && {LIFE_SETTINGS(getNumber,"enable_fatigue") isEqualTo 1}) then {
                [] spawn {
                    ...
                    [(format [localize "STR_ISTR_RedGullEffect"]),"Hinweis","Green"] call MSG_fnc_handle;
    ...
    case (_item isEqualTo "spikeStrip"): {
        if (!isNull life_spikestrip) exitWith {[(format [localize "STR_ISTR_SpikesDeployment"]),"Hinweis","Yellow"] call MSG_fnc_handle; closeDialog 0};
        if ([false,_item,1] call life_fnc_handleInv) then {
    ...
    default {
        [(format [localize "STR_ISTR_NotUsable"]),"Hinweis","Yellow"] call MSG_fnc_handle;
    };
};
