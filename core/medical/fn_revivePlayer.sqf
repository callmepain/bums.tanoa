...
if (_target getVariable ["Reviving",objNull] != player) exitWith {[(format [localize "STR_Medic_AlreadyReviving"]),"Hinweis","Yellow"] call MSG_fnc_handle;};
_target setVariable ["Reviving",NIL,true];

if (!alive player || life_istazed || life_isknocked) exitWith {life_action_inUse = false;};
if (_target getVariable ["Revive",false]) exitWith {[(format [localize "STR_Medic_RevivedRespawned"]),"Hinweis","Yellow"] call MSG_fnc_handle;};
if (player getVariable ["restrained",false]) exitWith {life_action_inUse = false;};
if (!isNil "_badDistance") exitWith {[(format [localize "STR_Medic_TooFar"]),"Hinweis","Yellow"] call MSG_fnc_handle;; life_action_inUse = false;};
if (life_interrupted) exitWith {life_interrupted = false; [(format [localize "STR_NOTF_ActionCancel"]),"Hinweis","Yellow"] call MSG_fnc_handle;; life_action_inUse = false;};
...
if (playerSide isEqualTo independent) then {
    [(format [localize "STR_Medic_RevivePayReceive",_targetName,[_reviveCost] call life_fnc_numberText]),"Hinweis","Yellow"] call MSG_fnc_handle;
