...
if (!(_diff isEqualTo (parseNumber _val))) then {
    if ([true,_item,_diff] call life_fnc_handleInv) then {
        [(format [localize "STR_MISC_TooMuch_3",_from getVariable ["realname",name _from],_val,_diff,((parseNumber _val) - _diff)]),"Hinweis","Yellow"] call MSG_fnc_handle;
    ...
    if ([true,_item,(parseNumber _val)] call life_fnc_handleInv) then {
        ...
        [(format [localize "STR_NOTF_GivenItem",_from getVariable ["realname",name _from],_val,(localize _type)]),"Hinweis","Green"] call MSG_fnc_handle;
    } else {
        [_from,_item,_val,_unit,false] remoteExecCall ["life_fnc_giveDiff",_from];
    };
};
