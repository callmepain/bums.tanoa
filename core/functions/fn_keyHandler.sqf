...
switch (_code) do {
	...
	////holzfällen
	case 16: {
		if (!life_action_inUse AND ((vehicle player) isEqualTo player) AND ((cursorObject distance2D player) < 10) AND (((getModelInfo cursorobject) select 0) in life_wood)) then {
			[] spawn  {
				private "_handle";
        ...
        if (!_alt && !_ctrlKey && !dialog && {!life_action_inUse}) then {
            if (!(isNull objectParent player) && alive vehicle player) then {
                ...
                if (!(isNil "_list")) then {
                    ...
                    if ((KINDOF_ARRAY(cursorObject,_list) || KINDOF_ARRAY(cursorObject,life_Container)) && {player distance cursorObject < 7} && {isNull objectParent player} && {alive cursorObject} && {!life_action_inUse}) then {
    ...
    case 33: {
        if (playerSide in [west,independent] && {vehicle player != player} && {!life_siren_active} && {((driver vehicle player) == player)}) then {
            ...
            if ((_veh getVariable "siren")) then {
                [(format [localize "STR_MISC_SirensOFF"]),"Hinweis","Green"] call MSG_fnc_handle;
                _veh setVariable ["siren",false,true];
            } else {
                [(format [localize "STR_MISC_SirensON"]),"Hinweis","Green"] call MSG_fnc_handle;
    ...
    case 24: {
        if (_shift) then {
            if !(soundVolume isEqualTo 1) then {
                1 fadeSound 1;
				[(format [localize "STR_MISC_soundnormal"]),"Hinweis","Green"] call MSG_fnc_handle;
            } else {
                1 fadeSound 0.1;
				[(format [localize "STR_MISC_soundfade"]),"Hinweis","Green"] call MSG_fnc_handle;
            };
        };
    };
            ...
            if (_veh isKindOf "House_F" && {playerSide isEqualTo civilian}) then {
                if (_veh in life_vehicles && {player distance _veh < 20}) then {
                    ...
                    if (_locked isEqualTo 0) then {
						...
						[(format [localize "STR_House_Door_Lock"]),"Hinweis","Green"] call MSG_fnc_handle;
                    } else {
                        _veh setVariable [format ["bis_disabled_Door_%1",_door],0,true];
                        _veh animateSource [format ["Door_%1_source", _door], 1];
						[(format [localize "STR_House_Door_Unlock"]),"Hinweis","Green"] call MSG_fnc_handle;
                    };
                };
                ...
                if (_veh in life_vehicles && {player distance _veh < 20}) then {
                    if (_locked isEqualTo 2) then {
						...
						[(format [localize "STR_MISC_VehUnlock"]),"Hinweis","Green"] call MSG_fnc_handle;
						...
						[(format [localize "STR_MISC_VehLock"]),"Hinweis","Green"] call MSG_fnc_handle;
