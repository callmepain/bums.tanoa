...
if ((time - life_action_delay) < 2) exitWith {[(format [localize "STR_NOTF_ActionDelay"]),"Hinweis","Yellow"] call MSG_fnc_handle;; INUSE(_this);};
...
if (_diff <= 0) exitWith {[(format [localize "STR_NOTF_InvFull"]),"Hinweis","Yellow"] call MSG_fnc_handle;; INUSE(_this);};
...
if (!(_diff isEqualTo (_itemInfo select 1))) then {
    if ([true,(_itemInfo select 0),_diff] call life_fnc_handleInv) then {
        ...
        [(format [localize "STR_NOTF_Picked",_diff,_itemName]),"Hinweis","Green"] call MSG_fnc_handle;
        INUSE(_this);
    } else {
        INUSE(_this);
    };
