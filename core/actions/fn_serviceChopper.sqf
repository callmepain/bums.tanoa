...
if (life_action_inUse) exitWith {[(format [localize "STR_NOTF_Action"]),"Hinweis","Yellow"] call MSG_fnc_handle;};

_serviceCost = LIFE_SETTINGS(getNumber,"service_chopper");
_search = nearestObjects[getPos air_sp, ["Air"],10];

if (count _search isEqualTo 0) exitWith {[(format [localize "STR_Service_Chopper_NoAir"]),"Hinweis","Yellow"] call MSG_fnc_handle;};
if (CASH < _serviceCost) exitWith {[(format [localize "STR_Serive_Chopper_NotEnough"]),"Hinweis","Yellow"] call MSG_fnc_handle;};
...
[(format [localize "STR_Service_Chopper_Done"]),"Hinweis","Green"] call MSG_fnc_handle;
