...
private _tanoaArray = ["Land_School_01_F","Land_Warehouse_03_F","Land_House_Small_02_F","Land_temple_native_01_f","Land_slum_03_f","Land_house_big_03_f"];
...
if (isNil {group player getVariable "gang_name"}) exitWith {[(format [localize "STR_GNOTF_CreateGang"]),"Hinweis","Yellow"] call MSG_fnc_handle;};
if (_group == group player) exitWith {[(format [localize "STR_GNOTF_Controlled"]),"Hinweis","Yellow"] call MSG_fnc_handle;};
if ((_hideout getVariable ["inCapture",false])) exitWith {[(format [localize "STR_GNOTF_onePersonAtATime"]),"Hinweis","Yellow"] call MSG_fnc_handle;};
...
if (!isNil "_action" && {!_action}) exitWith {[(format [localize "STR_GNOTF_CaptureCancel"]),"Hinweis","Yellow"] call MSG_fnc_handle;};
...
if (life_interrupted) exitWith {life_interrupted = false; [(format [localize "STR_GNOTF_CaptureCancel"]),"Hinweis","Yellow"] call MSG_fnc_handle; life_action_inUse = false;_hideout setVariable ["inCapture",false,true];};
life_action_inUse = false;
[(format [localize "STR_GNOTF_Captured"]),"Hinweis","Green"] call MSG_fnc_handle;
