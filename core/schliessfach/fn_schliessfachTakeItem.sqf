...
if(isNull life_schliessfach OR !alive life_schliessfach) exitWith {[(format [localize "STR_MISC_VehDoesntExist"]),"Hinweis","Yellow"] call MSG_fnc_handle;};
if(!alive player) exitwith {closeDialog 0;};

if((lbCurSel 4502) == -1) exitWith {[(format [localize "STR_Global_NoSelection"]),"Hinweis","Yellow"] call MSG_fnc_handle;};
_state = [_this,0,false] call bis_fnc_param;
_ctrl = ctrlSelData(4502);
_num = ctrlText 4505;
...
_index = [_ctrl,((life_schliessfach getVariable "schliessfach") select 0)] call TON_fnc_index;
_data = (life_schliessfach getVariable "schliessfach") select 0;
_old = life_schliessfach getVariable "schliessfach";
...
if(_state) then
{
	...
	if([true,_ctrl,_num] call life_fnc_handleInv) then
	{
		...
		life_schliessfach setVariable["schliessfach",[_data,(_old select 1) - _weight],true];
