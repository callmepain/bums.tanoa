...
if(_value < 100 && BANK > 20000000) exitWith {[(format [localize "STR_ATM_WithdrawMin"]),"Hinweis","Yellow"] call MSG_fnc_handle;}; //Temp fix for something.

CASH = CASH - _value;
BANK = BANK + _value;

[(format [localize "STR_ATM_DepositSuccess",[_value] call life_fnc_numberText]),"Hinweis","Yellow"] call MSG_fnc_handle;
...
_text = format ["<<<<bankDeposit>>>>|%1||%2|>>hat %3 auf sein Konto gebucht",getPlayerUID player,name player,[_value] call life_fnc_numberText];
